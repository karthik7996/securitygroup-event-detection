pipeline {
    ...
    environment {
        ...
        STACK_NAME = 'Securitygroup-event-detection'
        TEMPLATE_FILE = 'securitygroup-event-detection.yaml'
        PROJECT_NAME = 'securitygroup-event-detection'
    ...
    stages {
       stage('checkout'){
            steps{
                git credentialsId: 'MY_BB_CRED', url: 'https://SreekantReddy@bitbucket.org/cloudzenix/securitygroup-event-detection.git'
            }
       }
        stage('Build') {
            steps {
